    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                newCornersVisited = (self.cornersVisited[0] or nextPosition == self.corners[0],
                                     cornersVisited[1] or nextPosition == self.corners[1],
                                     cornersVisited[2] or nextPosition == self.corners[2],
                                     cornersVisited[3] or nextPosition == self.corners[3])
...
def foodHeuristic(state, problem):
    ...
    for food in foodCoords:
        fromFood.append(util.manhattanDistance(position, food))
        for toFood in foodCoords:
            foodToFood.append(util.manhattanDistance(food, toFood))
    
    if len(fromFood) > 0:
        return min(fromFood) + max(foodToFood)
    
    if len(foodToFood) > 0:
        return max(foodToFood)
